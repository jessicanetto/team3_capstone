...
(function () {
  ...
  function U(a, b) {
    ...
    for (b = { text: "", bytes: [], chunks: [], version: b }; 4 <= a.available(); ) {
      ...
      if (d === t.ECI) 0 === a.readBits(1) ? b.chunks.push({ type: r.ECI, assignmentNumber: a.readBits(7) }) : 0 === a.readBits(1) ? b.chunks.push({ type: r.ECI, assignmentNumber: a.readBits(14) }) : 0 === a.readBits(1) ? b.chunks.push({ type: r.ECI, assignmentNumber: a.readBits(21) }) : b.chunks.push({ type: r.ECI, assignmentNumber: -1 });
      else if (d === t.Numeric) {
        var e = a;
        d = [];
        ...
        d = [];
        f = "";
        for (g = e.readBits([9, 11, 13][c]); 2 <= g; ) (n = e.readBits(11)), (k = Math.floor(n / 45)), (n %= 45), d.push(B[k].charCodeAt(0), B[n].charCodeAt(0)), (f += B[k] + B[n]), (g -= 2);
  ...
  function Z(a, b, c) {
    ...
    for (var g of b.alignmentPatternCenters) for (var h of b.alignmentPatternCenters) (6 === g && 6 === h) || (6 === g && h === e - 7) || (g === e - 7 && 6 === h) || f.setRegion(g - 2, h - 2, 5, 5, !0);
  ...
  function ea(a, b, c) {
    ...
    d.ecBlocks.forEach((a) => {
      for (let b = 0; b < a.numBlocks; b++) e.push({ numDataCodewords: a.dataCodewordsPerBlock, codewords: [] }), (f += a.dataCodewordsPerBlock + d.ecCodewordsPerBlock);
    });
  ...
  function M(a, b, c, d) {
    ...
    if (0 === e && 0 === f) return { a11: b.x - a.x, a12: b.y - a.y, a13: 0, a21: c.x - b.x, a22: c.y - b.y, a23: 0, a31: a.x, a32: a.y, a33: 1 };
    {
      let h = b.x - c.x;
      var g = d.x - c.x;
      let k = b.y - c.y,
      ...
      g = (e * n - g * f) / c;
      e = (h * f - e * k) / c;
      return { a11: b.x - a.x + g * b.x, a12: b.y - a.y + g * b.y, a13: g, a21: d.x - a.x + e * d.x, a22: d.y - a.y + e * d.y, a23: e, a31: a.x, a32: a.y, a33: 1 };
    }
  }
  function fa(a, b, c, d) {
    a = M(a, b, c, d);
  ...
  function N(a, b, c, d) {
    ...
    for (let v = g, u = h; v !== a + l; v += l) {
      g = f ? u : v;
      h = f ? v : u;
      if (c.get(g, h) !== q && ((q = !q), e.push({ x: g, y: h }), e.length === d + 1)) break;
      m += n;
      if (0 < m) {
        if (u === b) break;
        u += p;
  ...
  function ka(a) {
    ...
    for (let m = 0; m <= a.height; m++) {
      ...
      for (let b = -1; b <= a.width; b++) {
        ...
        if (h === g) f++;
          ...
          g = h;
          var k = x(l) / 7;
          k = Math.abs(l[0] - k) < k && Math.abs(l[1] - k) < k && Math.abs(l[2] - 3 * k) < 3 * k && Math.abs(l[3] - k) < k && Math.abs(l[4] - k) < k && !h;
          var n = x(l.slice(-3)) / 3;
          ...
          if (k) {
            ...
            n = c.filter((b) => (d >= b.bottom.startX && d <= b.bottom.endX) || (a >= b.bottom.startX && d <= b.bottom.endX) || (d <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX - b.bottom.startX) && 0.5 < l[2] / (b.bottom.endX - b.bottom.startX)));
          ...
          if (h) {
            ...
            k = e.filter((b) => (c >= b.bottom.startX && c <= b.bottom.endX) || (a >= b.bottom.startX && c <= b.bottom.endX) || (c <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX - b.bottom.startX) && 0.5 < l[2] / (b.bottom.endX - b.bottom.startX)));
    ...
    for (b = 0; b < Math.min(c.length, 5); ++b) {
      e = c[b];
      f = [];
      for (var l of c) l !== e && f.push(Object.assign(Object.assign({}, l), { score: l.score + Math.pow(l.size - e.size, 2) / e.size }));
    ...
    b = H(a, q);
    c = H(a, v);
    (a = P(a, d, m, b, c)) && l.push({ alignmentPattern: { x: a.alignmentPattern.x, y: a.alignmentPattern.y }, bottomLeft: { x: c.x, y: c.y }, topLeft: { x: b.x, y: b.y }, topRight: { x: m.x, y: m.y }, dimension: a.dimension });
    ...
    b = b
      .map((b) => {
        ...
        if (a.get(Math.floor(c), Math.floor(b))) {
          var d = O({ x: Math.floor(c), y: Math.floor(b) }, [1, 1, 1], a) + y({ x: c, y: b }, n);
  ...
  function I(a, b, c, d = {}) {
    ...
    else for (l = 0; l < c; l++) for (p = 0; p < b; p++) (q = 4 * (l * b + p)), m.set(p, l, g.red * a[q] + g.green * a[q + 1] + g.blue * a[q + 2]);
    g = Math.ceil(b / 8);
    l = Math.ceil(c / 8);
    p = g * l;
    if (h) {
    ...
    h ? ((p = new Uint8ClampedArray(a.buffer, n, k)), (n += k), (p = new A(p, b))) : (p = A.createEmpty(b, c));
    q = null;
    ...
    (f = Q(d ? x : w)) || ("attemptBoth" !== e.inversionAttempts && "invertFirst" !== e.inversionAttempts) || (f = Q(d ? w : x));
    return f;
  }
  class A {
    constructor(a, b) {
  ...
  class w {
    constructor(a, b) {
      ...
      if (1 < c && 0 === b[0]) {
        ...
        else for (this.coefficients = new Uint8ClampedArray(c - d), a = 0; a < this.coefficients.length; a++) this.coefficients[a] = b[d + a];
  ...
  let K = [
    ...
    la = { inversionAttempts: "attemptBoth", greyScaleWeights: { red: 0.2126, green: 0.7152, blue: 0.0722, useIntegerApproximation: !1 }, canOverwriteImage: !0 };
  I.default = I;
  let G = "dontInvert",
